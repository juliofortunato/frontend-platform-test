...
const SongCard = ({ disableFavorite = false, song }: SongCardProps) => {
  return (
    <Link
      className="w-full max-w-[283px] overflow-hidden rounded-md bg-[#262626] transition-colors hover:bg-white/25"
      href={`/songs/${song.id}`}
    >
      ...
      <div className="relative flex items-end justify-between p-4">
        <div className="max-w-full">
          ...
          <h4 className="truncate text-xs font-medium text-[#666666]">
            {song.song.artist}
          </h4>
        </div>
        {!disableFavorite && (
